            ...
            <View className="flex-1">
                <ScrollView className="flex-1">
                    <View className="p-6 flex-row justify-between items-center">
                        <View>
                            <Text className="text-3xl font-extrabold text-success-dark dark:text-success-light tracking-tight">Your Strengths</Text>
                            <Text className="text-base text-success dark:text-success-light mt-1">Chapters where you are excelling</Text>
                        </View>
                        <TouchableOpacity onPress={fetchData} className="p-2.5 rounded-full bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark shadow-sm">
                            <Ionicons name="refresh" size={20} color={isDark ? "#34d399" : "#059669"} />
                        </TouchableOpacity>
                    </View>
                    ...
                    {loading ? (
                        <View className="p-20 items-center">
                            ...
                            <Text className="mt-4 text-[15px] text-success dark:text-success-light font-medium">Analyzing your success...</Text>
                        </View>
                    ) : (
                        <View className="px-6 pb-6 gap-6">
                            <View className="flex-row gap-4 mb-2">
                                <View className="flex-1 bg-card-light dark:bg-card-dark p-5 rounded-2xl border border-border-light dark:border-border-dark shadow-sm">
                                    ...
                                    <Text className="text-2xl font-bold text-success dark:text-success-light mt-1">{totalAttempts}</Text>
                                </View>
                                <View className="flex-1 bg-card-light dark:bg-card-dark p-5 rounded-2xl border border-border-light dark:border-border-dark shadow-sm">
                                    ...
                                    <Text className="text-2xl font-bold text-success dark:text-success-light mt-1">{strengths.length}</Text>
                                </View>
                            </View>
                                    ...
                                    <View className="gap-3">
                                        {strengths.map((item, index) => (
                                            <View key={index} className="bg-card-light dark:bg-card-dark p-4 rounded-2xl flex-row items-center border border-border-light dark:border-border-dark shadow-sm">
                                                <View className="w-10 h-10 rounded-full bg-success/10 dark:bg-success/20 justify-center items-center mr-4">
                                                    <Text className="text-success dark:text-success-light font-extrabold text-sm">#{index + 1}</Text>
                                                </View>
                                                <View className="flex-1">
                                                    ...
                                                    <View className="flex-row items-center mt-2 gap-2.5">
                                                        <View className="flex-1 h-1.5 bg-background-light dark:bg-background-dark rounded-full overflow-hidden">
                                                            <View className="h-full bg-success dark:bg-success-light rounded-full" style={{ width: `${item.accuracy}%` }} />
                                                        </View>
                                                        <Text className="text-sm font-bold text-success dark:text-success-light w-10">{Math.round(item.accuracy)}%</Text>
                                                    </View>
                                                </View>
                                                <View className="ml-4">
                                                    <MaterialCommunityIcons name="shield-check" size={28} color={isDark ? "#34d399" : "#10b981"} />
                                                </View>
                                            </View>
                                        ))}
                                    </View>
