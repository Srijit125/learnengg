...
export default function Index() {
    ...
    <View className="flex-row bg-card-light dark:bg-card-dark mx-6 rounded-xl p-1.5 shadow-sm shadow-[#000]/10 border border-divider-light dark:border-divider-dark">
      {TABS.map(tab => {
        ...
        return (
          <TouchableOpacity
            key={tab.id}
            className={`flex-1 flex-row items-center justify-center py-3 gap-2 rounded-lg ${isActive ? "bg-[#f0f4ff]" : ""}`}
          ...
          >
            ...
            />
            <Text className={`text-sm font-semibold ${isActive ? "color-[#667eea]" : "text-textSecondary-light dark:text-textSecondary-dark"}`}>
              {tab.title}
            </Text>
          </TouchableOpacity>
        );
      ...
      <View className="flex-row flex-wrap gap-4 mb-6">
        {coursesStats.map(course => (
          <View key={course.id} className="bg-card-light dark:bg-card-dark p-5 rounded-2xl border border-divider-light dark:border-divider-dark shadow-sm shadow-[#000]/5 gap-4" style={{ width: (Dimensions.get('window').width - 128) / 3 }}>
            ...
            >
              <Text className="text-sm font-semibold color-[#667eea]">View Chapters</Text>
  ...
  const renderCourseDetailsModal = () => (
    ...
    >
      <View className="flex-1 bg-[#0f172a]/50 justify-center items-center p-10">
        <View className="bg-card-light dark:bg-card-dark rounded-3xl w-[80%] max-w-[900px] max-h-[80%] overflow-hidden shadow-2xl shadow-[#000]/20">
            ...
            <ScrollView className="p-6">
              ...
              {selectedCourseDetails?.chapters?.map((ch: any, idx: number) => (
                <View key={idx} className={`flex-row items-center py-3 border-b border-border-light dark:border-border-dark ${idx % 2 === 0 ? "bg-background-light dark:bg-background-dark" : "bg-card-light dark:bg-card-dark"}`}>
                  ...
                  <View className="flex-1">
                    <View className={`self-start px-2 py-1 rounded-lg ${ch.avg_accuracy > 70 ? 'bg-[#ecfdf5]' : 'bg-[#fff7ed]'}`}>
                      <Text className={`text-[13px] font-bold ${ch.avg_accuracy > 70 ? 'color-[#059669]' : 'color-[#d97706]'}`}>
                        {ch.avg_accuracy}%
                      </Text>
                    </View>
                  </View>
      ...
      <LinearGradient colors={["#f8fafc", "#f1f5f9"]} className="flex-1">
        <ScrollView className="flex-1" stickyHeaderIndices={[1]}>
          <View className="px-6 pt-10 pb-5 flex-row justify-between items-center">
            ...
            <TouchableOpacity onPress={loadTabContent} className="p-2.5 rounded-[20px] bg-card-light dark:bg-card-dark shadow-sm shadow-[#000]/10 border border-divider-light dark:border-divider-dark">
